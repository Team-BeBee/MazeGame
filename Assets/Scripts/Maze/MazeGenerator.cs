...
public class MazeGenerator
{
    ...
    {
        MazeGrid grid = new MazeGrid(config.Width, config.Height);

        Random rng = config.UseRandomSeed ? new Random() : new Random(config.Seed);
    ...
    private void DFS(MazeGrid grid, int x, int y, Random rng)
    {
        ...
        foreach ((int dx, int dy) in dirs)
        {
            ...
            MazeCell nextCell = grid.GetCell(nx, ny);
